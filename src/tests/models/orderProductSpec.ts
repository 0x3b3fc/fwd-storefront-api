...
describe("Tests the OrderProduct Model", () => {
  ...
  it("adds order products with the create method", async () => {
    ...
    expect(parseInt(orderProduct1.order_id as unknown as string)).toEqual(
      order1.id as number
    );
    expect(parseInt(orderProduct1.product_id as unknown as string)).toEqual(
      product1.id as number
    );
    ...
    expect(parseInt(orderProduct2.order_id as unknown as string)).toEqual(
      order2.id as number
    );
    expect(parseInt(orderProduct2.product_id as unknown as string)).toEqual(
      product2.id as number
    );
  ...
  it("returns a list of order products with the index method", async () => {
    ...
    expect(parseInt(result[1].order_id as unknown as string)).toEqual(
      order2.id as number
    );
    expect(parseInt(result[1].product_id as unknown as string)).toEqual(
      product2.id as number
    );
  ...
  it("shows the order products with the show method", async () => {
    let result = await orderProductStore.show(
      orderProduct1.id as unknown as string
    );
