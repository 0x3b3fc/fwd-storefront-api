...
describe("Tests the Order Model", () => {
  ...
  it("adds an order with the create method", async () => {
    ...
    expect(parseInt(order1.user_id as unknown as string)).toEqual(
      user1.id as number
    );
    ...
    expect(parseInt(order2.user_id as unknown as string)).toEqual(
      user2.id as number
    );
  ...
  it("returns a list of all orders with the index method", async () => {
    ...
    expect(parseInt(result[0].user_id as unknown as string)).toEqual(
      user1.id as number
    );
    expect(result[0].status).toEqual("active");

    expect(parseInt(result[1].user_id as unknown as string)).toEqual(
      user2.id as number
    );
  ...
  it("shows all orders of a user with the show method", async () => {
    ...
    expect(parseInt(plainResult.id as unknown as string)).toEqual(
      order1.id as number
    );
    expect(parseInt(plainResult.user_id as unknown as string)).toEqual(
      user1.id as number
    );
